...
model WaterCooledChillerExample7 "Primary only chiller plant system with water-side economizer"
  ...
  Modelica.Blocks.Sources.Constant pumCHWcon2(k = mCHW_flow_nominal) annotation(
    Placement(visible = true, transformation(origin = {-46, 69}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Buildings.Fluid.Actuators.Valves.TwoWayLinear val8_2(redeclare package Medium = MediumW, dpFixed_nominal = 14930 + 89580, dpValve_nominal = 20902, from_dp = true, m_flow_nominal = mCHW_flow_nominal, use_inputFilter = false, y_start = 1) annotation(
    Placement(visible = true, transformation(origin = {-12, 2}, extent = {{10, -10}, {-10, 10}}, rotation = 270)));  WaterCooledChillerExample.Buildings.Fluid.Sensors.TemperatureTwoPort TWCLeaTow3(m_flow_nominal=mCW_flow_nominal, redeclare package Medium=MediumW) annotation(Placement(transformation(origin = {74, 135}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
...
equation
  ...
  connect(pumCHWcon2.y, pumCHW2.m_flow_in) annotation(
    Line(points = {{-35, 69}, {-26, 69}, {-26, 70}, {-12, 70}}, color = {0, 0, 127}));
