model logic2 "Primary only chiller plant system with water-side economizer; load-based CHWST reset with constant max CHW pump flow"
  ...
  parameter Modelica.SIunits.MassFlowRate mCHWPumpMax = mCHW_flow_nominal "Constant CHW pump mass flow rate used whenever the pump is enabled (nominal per-chiller flow)";
  ...
  block ChillerLeavingTempPID "Timed PI controller that resets the chiller leaving water temperature (CHWST) to hold the CHW return temperature (CHWRT) near a target"
    ...
    parameter Real loadGain(unit = "K/W") = 1e-5 "CHWST reduction gain from unmet coil load";
  ...
  Modelica.Blocks.Sources.RealExpression mCHWFlow1Cmd(y = if chillerStagingCtrl.chiller1On then mCHWPumpMax else 0) "Constant CHW pump 1 flow command: maximum flow when chiller 1 is enabled, zero otherwise" annotation(
  ...
  Modelica.Blocks.Sources.RealExpression mCHWFlow2Cmd(y = if chillerStagingCtrl.chiller2On then mCHWPumpMax else 0) "Constant CHW pump 2 flow command: maximum flow when chiller 2 is enabled, zero otherwise" annotation(
  ...
  ChillerLeavingTempPID chwstCtrl1(TCHWRTSet = TCHWRTSet, dTDeadband = 0.2, internalSamplePeriod = 5 * 60, externalSamplePeriod = 10 * 60, tempResponseTime = 120, TChiSetMin = TChiSetMin, TChiSetMax = TChiSetMax, TChiSetStart = TChiSetMin, kP = 1, Ti = 15 * 60, Td = 0) "CHWST reset controller for chiller 1" annotation(
    Placement(visible = true, transformation(origin = {382, 34}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
  ChillerLeavingTempPID chwstCtrl2(TCHWRTSet = TCHWRTSet, dTDeadband = 0.2, internalSamplePeriod = 5 * 60, externalSamplePeriod = 10 * 60, tempResponseTime = 120, TChiSetMin = TChiSetMin, TChiSetMax = TChiSetMax, TChiSetStart = TChiSetMin, kP = 1, Ti = 15 * 60, Td = 0) "CHWST reset controller for chiller 2" annotation(
...
equation
  ...
  connect(chwstCtrl1.TCHWSTSet, chi.TSet) annotation(
  ...
  connect(chillerStagingCtrl.chiller1On, chwstCtrl1.pumpOn) annotation(
    Line(points = {{340, 144}, {382, 144}, {382, 34}}, color = {255, 0, 255}));
  connect(coilCoolingSignal.y, chwstCtrl1.coilCooling);
  connect(roomDemandSignal.y, chwstCtrl1.coolingDemand);
  ...
  connect(chwstCtrl2.TCHWSTSet, chi2.TSet) annotation(
  ...
  connect(chillerStagingCtrl.chiller2On, chwstCtrl2.pumpOn) annotation(
    Line(points = {{340, 136}, {346, 136}, {346, -36}, {370, -36}}, color = {255, 0, 255}));
  connect(coilCoolingSignal.y, chwstCtrl2.coilCooling);
  connect(roomDemandSignal.y, chwstCtrl2.coolingDemand);
  connect(TCHWChi1In.T, chwstCtrl1.TCHWReturn) annotation(
    Line(points = {{242, 98}, {206, 98}, {206, 45}, {382, 45}, {382, 34}}, color = {0, 0, 127}));
  connect(TCHWChi2In.T, chwstCtrl2.TCHWReturn) annotation(
